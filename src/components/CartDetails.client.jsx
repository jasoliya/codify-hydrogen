...
function CartEmpty({ onClose }) {
	return (
		<div className="flex flex-col space-y-7 justify-center items-center md:py-8 md:px-12 px-4 py-6 h-screen">
			...
			<button className="inline-block rounded-sm text-center py-3 px-3 max-w-xl bg-black text-white w-full" onClick={onClose}>Continue Shopping</button>
		</div>
	)
}
	...
	return (
		<div className="flex flex-col items-center mt-6 md:mt-8">
			<Link 
				to={checkoutUrl}
				width="full"
				className="inline-block rounded-sm font-medium text-center py-3 px-6 max-w-xl bg-black text-white w-full"
...
function CartLineItem() {
	...
	const { id, quantity, merchandise } = useCartLine();
	...
	return (
		<>
			...
			<div className="flex items-center overflow-auto border rounded">
				<CartLineQuantityAdjustButton
					adjust="decrease"
					className="h-[40px] flex justify-center items-center px-3 py-[0.125rem] transition text-primary/40 hover:text-primary disabled:pointer-event-all disabled:cursor-wait">
					&#8722;
				</CartLineQuantityAdjustButton>
				<CartLineQuantity 
					as="div"
